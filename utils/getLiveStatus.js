...
@returns {Object|null} timeSinceStart - The time since the campaign started, or null if start date is in future or unpublished.
...
const getLiveStatus = (campaign) => {
  ...
  const isFinished = campaign.published && now >= endDate;

  const timeUntilEnd = isLive ? getTimeDifference(now, endDate) : null;
  const timeSinceStart =
    !startsInFuture && !isFinished && campaign.published
